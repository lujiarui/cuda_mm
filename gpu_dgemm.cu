/*THIS IS TEST FOR GPU IMPLEMENTAION OF DOUBLE-PRECISION MAT-MAT MULTIPICATION
...
#define EPSILON 0.0001 // the error limit for identification
...
void mat_init(double *mat,int row, int col)
{
    ...
    for(i=0;i<row;i++)
    {
        for(j=0;j<col;j++)
        {
            // random double-precision value assignment
            mat[(i*col) + j] = (double)rand() / (double)RAND_MAX + ( (i+j)%2==1?1:-1 );
            //mat[(i*col) + j] = 1.0;
        }
    }
}
// gpu kernel of multiplication
__global__ void mat_mul(double *lmat, double *rmat, double *res_mat, int m, int n)
{
...
int main(int argc, char *argv[])
{
    ...
    double *lmat, *rmat, *res_mat;
    double *g_lmat, *g_rmat, *g_res_mat;
    // allocation of the memory
    lmat    = (double *)malloc(sizeof(double) * m * n);
    rmat    = (double *)malloc(sizeof(double) * n * m);
    res_mat = (double *)malloc(sizeof(double) * m * m);
    //random seed setting
    ...
    cudaMalloc((void **)&g_lmat,    sizeof(double) * m * n);
    cudaMalloc((void **)&g_rmat,    sizeof(double) * n * m);
    cudaMalloc((void **)&g_res_mat, sizeof(double) * m * m);
    
    gettimeofday(&start,NULL);
    //copy data to v-memory
    cudaMemcpy(g_lmat,lmat, sizeof(double) * m * n, cudaMemcpyHostToDevice);
    cudaMemcpy(g_rmat,rmat, sizeof(double) * n * m, cudaMemcpyHostToDevice);
    //printf("This is a test sentence[%d]\n",test_count++);
    ...
    cudaMemcpy(res_mat, g_res_mat, sizeof(double) * m * m, cudaMemcpyDeviceToHost);
